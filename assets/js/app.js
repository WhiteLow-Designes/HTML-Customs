(function () {
  ...
  var detectedGames = [];
  var localGameScan = { status: 'loading', roots: [], error: '' };
  ...
  var APP_DEFS = {
    home: { name: 'Startseite', title: 'Startseite - Datei-Explorer', icon: '🖥️', kind: 'desktop' },
    media: { name: 'Bilder & Videos', title: 'Bilder & Videos', icon: '🖼️', kind: 'media' },
  ...
  var DEFAULT_STATE = {
    ...
    recent: [
      { app: 'media', name: 'Samurai Wallpaper.png', detail: 'Bild' },
  ...
  function renderWindowBody(node) {
    ...
    else if (['media', 'games', 'trash'].indexOf(app) !== -1) body.innerHTML = renderExplorer(app, '', node);
  ...
  function refreshExplorers() {
    ['home', 'media', 'games', 'trash'].forEach(refreshWindow);
    renderDesktopIcons();
  ...
  function explorerToolbar(app, folderId) {
    ...
    if (app === 'home') {
      ...
      parts.push(commandButton('upload', 'Importieren', 'import-media'));
      parts.push(commandButton('link', 'URL einbetten', 'embed-media'));
      parts.push(commandButton('eye', 'Öffnen', 'open-selected', !currentSelection));
      parts.push(commandButton('trash', 'Papierkorb', 'delete-media', !currentSelection));
    } else if (app === 'games') {
      parts.push(commandButton('plus', 'Spiel hinzufügen', 'add-game'));
      parts.push(commandButton('refresh', 'PC durchsuchen', 'scan-games'));
  ...
  function isExplorerApp(app) {
    return ['home', 'media', 'games', 'trash'].indexOf(app) !== -1;
  }
  ...
  function selectedItemFor(app, folderId) {
    ...
    if (app === 'media') return state.media.find(function (item) { return item.id === selectedFileId; }) || null;
  ...
  function explorerSidebar(active) {
    return '<nav class="explorer-sidebar" aria-label="Explorer-Navigation">' +
      sidebarButton('home', '🏠', 'Startseite', active) +
      sidebarButton('home', '🖥️', 'Desktop', active) +
      sidebarButton('media', '🖼️', 'Bilder & Videos', active) +
  ...
  function itemCount(app, folderId) {
    if (app === 'home') {
      ...
      return count + (folderId ? 0 : 4);
    }
    if (app === 'media') return state.media.length;
  ...
  function renderExplorerContent(app, query, folderId) {
    ...
    if (app === 'media') return renderMedia(query);
  ...
  function renderHomeFiles(query, folderId) {
    ...
    var folders = folderId ? [] : [
      { id: 'virtual-media', name: 'Bilder & Videos', type: 'folder', app: 'media' },
  ...
  function renderMedia(query) {
    var items = state.media.filter(function (item) { return matchesQuery(item, query); });
    if (!items.length) return emptyState('🖼️', 'Noch keine Medien', 'Importiere lokale Dateien oder bette Bilder und Videos über eine URL ein.');
    if (state.settings.mediaView === 'list') {
      return '<div class="content-heading"><h2>Bilder & Videos</h2><small>' + items.length + ' Medien</small></div><table class="file-list"><thead><tr><th>Name</th><th>Typ</th><th>Größe</th><th>Geändert</th></tr></thead><tbody>' + items.map(function (item) {
        return '<tr data-media-id="' + item.id + '" draggable="true"><td><span class="name-cell"><span>' + (item.type === 'video' || item.type === 'embed' ? '🎬' : '🖼️') + '</span>' + escapeHtml(item.name) + '</span></td><td>' + escapeHtml(mediaTypeLabel(item)) + '</td><td>' + escapeHtml(mediaSizeLabel(item)) + '</td><td>' + formatDate(item.modified) + '</td></tr>';
      }).join('') + '</tbody></table>';
    }
    return '<div class="content-heading"><h2>Bilder & Videos</h2><small>' + items.length + ' Medien</small></div><div class="media-grid">' + items.map(function (item) {
      var source = runtimeMedia.get(item.id) || item.src || '';
      var preview = item.type === 'embed' ? '<span class="media-embed-preview">▶</span>' : item.type === 'video' ? (source ? '<video src="' + escapeHtml(source) + '" preload="metadata" muted></video>' : '🎬') : (source ? '<img src="' + escapeHtml(source) + '" alt="">' : '🖼️');
      return '<button class="media-card' + (selectedFileId === item.id ? ' selected' : '') + '" type="button" draggable="true" data-media-id="' + item.id + '"><span class="media-preview">' + preview + '<span class="media-type-badge">' + escapeHtml(mediaBadgeLabel(item)) + '</span></span><span class="media-info"><strong>' + escapeHtml(item.name) + '</strong><span>' + escapeHtml(mediaSizeLabel(item)) + ' · ' + formatDate(item.modified) + '</span></span></button>';
    }).join('') + '</div>';
  }
  ...
  function mediaBadgeLabel(item) {
    return item.type === 'embed' ? 'WEBVIDEO' : item.type === 'video' ? 'VIDEO' : 'BILD';
  }
  ...
  function renderGames(query) {
    ...
    var scanCopy = localGameScan.status === 'loading' ? 'Lokale Spiele werden gesucht …' : localGameScan.status === 'ready' ? detectedGames.length + ' lokale Installationen erkannt' : localGameScan.error || 'Lokale Suche ist nur über den PHP-Server verfügbar.';
    return '<div class="content-heading"><div><h2>Spielebibliothek</h2><small class="library-scan">' + escapeHtml(scanCopy) + '</small></div><small>' + items.length + ' Games</small></div><div class="game-grid">' + items.map(function (game) {
      var source = game.detected ? (game.source === 'steam' ? 'STEAM' : 'XBOX') : 'MANUELL';
  ...
  function scanLocalGames(showResult) {
    if (location.protocol === 'file:') {
      localGameScan = { status: 'unavailable', roots: [], error: 'Starte die Webseite mit start-server.bat, damit Steam und Xbox eingelesen werden können.' };
      refreshWindow('games');
      return Promise.resolve([]);
    }
    localGameScan.status = 'loading';
    refreshWindow('games');
    return fetch('api/local-games.php', { cache: 'no-store', headers: { Accept: 'application/json' } }).then(function (response) {
      if (!response.ok) throw new Error('HTTP ' + response.status);
      return response.json();
    }).then(function (data) {
      if (!data || !data.ok || !Array.isArray(data.games)) throw new Error('Ungültige Antwort');
      detectedGames = data.games.map(function (game) {
        game.detected = true;
        return game;
      });
      localGameScan = { status: 'ready', roots: data.roots || [], error: '' };
      selectedFileId = detectedGameById(selectedFileId) ? null : selectedFileId;
      refreshWindow('games');
      refreshWindow('settings');
      if (showResult) toast('PC durchsucht', detectedGames.length + ' Steam-/Xbox-Einträge wurden erkannt.', '🎮');
      return detectedGames;
    }).catch(function () {
      detectedGames = [];
      localGameScan = { status: 'error', roots: [], error: 'Die lokalen Spieleordner konnten nicht gelesen werden.' };
      refreshWindow('games');
      if (showResult) toast('Suche nicht möglich', localGameScan.error, '⚠️');
      return [];
    });
  }
  ...
  function updateExplorerSelection(node) {
    var app = node.dataset.app;
    var actions = app === 'home' ? ['cut-file', 'copy-file', 'delete-file'] : app === 'media' ? ['open-selected', 'delete-media'] : app === 'games' ? ['edit-game', 'delete-game'] : app === 'trash' ? ['restore-selected', 'delete-permanent'] : [];
    actions.forEach(function (action) {
      var control = $('[data-action="' + action + '"]', node);
      if (control) control.disabled = app === 'games' && !!detectedGameById(selectedFileId);
    });
  ...
  function handleAction(action, button, node) {
    ...
    else if (action === 'import-media') $('#mediaInput').click();
    else if (action === 'embed-media') mediaUrlDialog();
    else if (action === 'media-view-grid' || action === 'media-view-list') {
      state.settings.mediaView = action.endsWith('list') ? 'list' : 'grid'; saveState(); refreshWindow('media');
    } else if (action === 'open-selected' && selectedFileId) openMedia(selectedFileId);
    else if (action === 'delete-media' && selectedFileId) deleteMedia(selectedFileId);
  ...
  function openVirtualFile(id) {
    ...
    showDialog({
      icon: '📄',
      title: file.name,
      message: file.content || 'Diese Textdatei ist leer.',
  ...
  function importMediaFiles(event) {
    ...
    files.forEach(function (file) {
      ...
      var item = { id: uid('media'), name: file.name, type: type, mime: file.type, size: file.size, modified: new Date(file.lastModified || Date.now()).toISOString(), src: '' };
      state.media.push(item);
      if (file.size <= 3.5 * 1024 * 1024) {
        var reader = new FileReader();
        reader.onload = function () { item.src = reader.result; finish(); };
        reader.onerror = finish;
        reader.readAsDataURL(file);
      } else {
        runtimeMedia.set(item.id, URL.createObjectURL(file));
        finish();
      }
    });
    function finish() {
      ...
      saveState(); refreshWindow('media'); refreshWindow('settings');
      toast('Medien importiert', files.length + ' Datei(en) wurden hinzugefügt.', '🖼️');
    }
  }
  ...
  function mediaUrlDialog() {
    showDialog({
      icon: '🖼️',
      title: 'Bild oder Video einbetten',
      message: 'Füge eine direkte Bild-/Video-Adresse oder einen YouTube-Link zur Medienbibliothek hinzu.',
      fields: [
        { name: 'name', label: 'Anzeigename', type: 'text', required: true },
        { name: 'kind', label: 'Medienart', type: 'select', value: 'image', options: [['image', 'Bild über URL'], ['video', 'Video-Datei über URL'], ['youtube', 'YouTube-Video']] },
        { name: 'url', label: 'Medien-Adresse', type: 'text', value: 'https://', required: true }
      ],
      ...
      if (!name || !address) return toast('Ungültige Medien-Adresse', 'Bitte gib eine gültige HTTP- oder HTTPS-Adresse ein.', '⚠️');
      ...
      state.media.push({
        id: uid('media'),
        name: name,
        type: type === 'video' ? 'video' : type === 'embed' ? 'embed' : 'image',
        mime: type === 'embed' ? 'text/html' : type === 'video' ? 'video/url' : 'image/url',
        size: 0,
      ...
      saveState(); refreshWindow('media'); refreshWindow('settings');
      toast('Medium eingebettet', name, type === 'image' ? '🖼️' : '🎬');
    });
  }
  ...
  function openMedia(id) {
    ...
    if (!item) return;
    var source = runtimeMedia.get(item.id) || item.src;
    if (!source) return toast('Datei nicht verfügbar', 'Große lokale Dateien müssen nach dem Neuladen erneut importiert werden.', '⚠️');
  ...
  function renderMediaViewer(node) {
    ...
    var source = runtimeMedia.get(item.id) || item.src;
    if (!source) {
      return '<div class="media-window-empty"><div><div class="empty-icon">⚠️</div><h3>Datei nicht verfügbar</h3><p>Große lokale Dateien müssen nach dem Neuladen erneut importiert werden.</p></div></div>';
    }
  ...
  function deleteMedia(id) {
    ...
    showDialog({ icon: '🗑️', title: 'Medium in den Papierkorb?', message: item.name + ' wird aus der Medienbibliothek entfernt und kann im Papierkorb wiederhergestellt werden.', confirmText: 'Verschieben', danger: true }).then(function (result) {
      if (!result) return;
      state.files.push({ id: uid('trash-media'), name: item.name, type: 'media-ref', location: 'trash', deletedFrom: 'Bilder & Videos', deletedAt: new Date().toISOString(), size: item.size, mediaSnapshot: item });
  ...
  function sortItems(app) {
    if (app === 'media') state.media.sort(function (a, b) { return a.name.localeCompare(b.name, 'de'); });
  ...
  function renderSettings(section) {
    ...
    return '<div class="settings-shell" data-settings-section="' + actual + '"><nav class="settings-nav"><div class="settings-profile-card"><span class="avatar">' + avatarMarkup() + '</span><span><strong>' + escapeHtml(state.profile.username) + '</strong><span>' + escapeHtml(state.profile.email) + '</span></span></div><label class="settings-search"><input type="search" placeholder="Einstellung suchen" aria-label="Einstellung suchen"></label>' +
      ...
      settingsNavButton('media', '🖼️', 'Bilder & Videos', actual) +
  ...
  function renderSettingsSection(section) {
    ...
    if (section === 'games') {
      ...
      return '<h1>Games</h1><p class="muted">Lokale Installationen werden ausschließlich lesend aus Steam und Xbox übernommen.</p>' +
        ...
        settingCard('Lokale Quellen', rootSummary || localGameScan.error || 'Die Quellen werden geprüft.', '<button class="button secondary small" type="button" data-action="scan-games">Erneut prüfen</button>');
    }
    if (section === 'media') {
      var total = state.media.reduce(function (sum, item) { return sum + Number(item.size || 0); }, 0);
      return '<h1>Bilder & Videos</h1>' +
        settingCard(state.media.length + ' Medien', formatBytes(total) + ' lokale Daten sowie eingebettete Webmedien.', '<button class="button primary small" type="button" data-action="import-media">Importieren</button><button class="button secondary small" type="button" data-action="embed-media">URL einbetten</button>') +
        settingCard('Ansicht', 'Standardansicht für den Medienordner.', '<select data-setting="mediaView"><option value="grid"' + (state.settings.mediaView === 'grid' ? ' selected' : '') + '>Kacheln / Galerie</option><option value="list"' + (state.settings.mediaView === 'list' ? ' selected' : '') + '>Liste</option></select>');
    }
    if (section === 'profile') {
    ...
    if (section === 'chat') {
      return '<h1>Raum-Chat</h1>' +
        settingCard('Echtzeit-Verbindung', chatSession.backend ? 'PHP/SQLite-Backend erreichbar.' : 'Offline-Modus aktiv. Starte den lokalen PHP-Server für Mehrbenutzer-Chat.', '<span class="' + (chatSession.backend ? 'success' : 'muted') + '">' + (chatSession.backend ? 'Verbunden' : 'Offline') + '</span>') +
    ...
    if (section === 'database') {
      return '<h1>Datenbank</h1>' +
        ...
        (state.settings.databaseEnabled ? protectedSettingsPanel([
          ['Verbindungsstatus', chatSession.backend ? 'PHP/SQLite verbunden' : 'Backend nicht erreichbar'],
          ['Datenbanktyp', 'SQLite mit PDO'],
          ['Lokaler Pfad', 'data/desktop.sqlite'],
    ...
    return '<h1>System</h1>' +
      ...
      settingCard('Backend', chatSession.backend ? 'PHP und SQLite sind verbunden.' : 'Frontend-Offline-Modus', '<span class="' + (chatSession.backend ? 'success' : 'muted') + '">' + (chatSession.backend ? 'Online' : 'Optional') + '</span>');
  }
  ...
  function clearLocalData() {
    showDialog({ icon: '⚠️', title: 'Alle lokalen Daten zurücksetzen?', message: 'Profil, Einstellungen, Dateien, Games und Medien werden auf den Auslieferungszustand gesetzt.', confirmText: 'Alles zurücksetzen', danger: true }).then(function (result) {
      if (!result) return;
  ...
  function sendChatFrom(node) {
    ...
    if (!body) return;
    if (body.charAt(0) === '/') {
      if (handleChatCommand(body, node)) { input.value = ''; return; }
    }
    if (chatSession.backend) {
      ...
      saveState(); input.value = ''; refreshWindow('chat');
    }
  }
  ...
  function pollChat(force) {
    ...
    apiRequest('messages', { room_id: state.chat.room, since: force ? 0 : chatSession.lastId }, 'GET').then(function (data) {
      ...
      if (!chatProfileState.editing) refreshWindow('chat');
    }).catch(function () {
